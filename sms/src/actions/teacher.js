...
export const addTeacher = (formData,config) => async (dispatch) =>{
    try {
        const {data} = await api.addTeacher(formData,config);
...
export const searchTeacher = (searchBy,search) => async (dispatch) =>{
    try {
        
        const {data} = await api.searchTeacher(searchBy,search);
        dispatch({type:SEARCH_TEACHER,payload:data})
